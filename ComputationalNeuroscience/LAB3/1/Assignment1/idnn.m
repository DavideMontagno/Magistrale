...
%% PARAM GRID
   nhs = [100 200 500];                 % hidden units
  wss = [10 20];                  % window size
...
for lambda = lambdas
    ...
    idnn_net = timedelaynet(0:ws, nh, 'traingdm');
...
[delayedInput_tr, initialInput_tr, initialStates_tr, delayedTarget_tr] = preparets(idnn_net, [X_tr X_val], [y_tr y_val]);

[delayedInput_ts, initialInput_ts, initialStates_ts, delayedTarget_ts] =  preparets(idnn_net, X_ts, y_ts);

% train on TR+VAL
[idnn_net, idnn_tr_record] = train(idnn_net, delayedInput_tr, delayedTarget_tr, initialInput_tr);
