...
while(true) %epochs
    ...
    for i=1:columns %get new w vector
        v=w'*dataset(:,i);
        theta_v=(theta_v+v)/2;
        delta = (v-theta_v)*(dataset(:,i)-theta_u);
...
savefig('./covariance/principal.fig');
...
xlabel('time');
title('w(1) over time');
savefig('./covariance/w1.fig');
figure
plot(w2)
xlabel('time');
title('w(2) component over time');
savefig('./covariance/w2.fig');
